...
class BNO055Node(Node):
    def __init__(self):
        ...
        self.timer = self.create_timer(1.0, self.publish_imu_data)  # 每 1 秒發佈一次 IMU 數據
    ...
    def publish_imu_data(self):
        ...
        imu_msg.linear_acceleration.x = 0.0  # 可根據需要填充加速度數據
        imu_msg.linear_acceleration.y = 0.0
        imu_msg.linear_acceleration.z = 0.0
        imu_msg.angular_velocity.x = math.radians(roll)
        imu_msg.angular_velocity.y = math.radians(pitch)
        imu_msg.angular_velocity.z = math.radians(heading)
        imu_msg.orientation.w = 1.0  # 設置四元數的預設值
        ...
        self.get_logger().info(f"Published IMU data: Heading: {heading:.2f}°, Roll: {roll:.2f}°, Pitch: {pitch:.2f}°")
